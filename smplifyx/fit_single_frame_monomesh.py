...
import fitting_monomesh as fitting
...
def fit_single_frame(img,
                     ...
                     keypoints3d = None, #shape(1,25,4)
                     mesh=None, # shape(6890,3)
                     **kwargs):
    # 条件为false则触发异常
    ...
    # keypoints3d gt 的形式转换
    use_3dpose = kwargs.get('use_3dpose', True)
    gt_joints3d=None
    joints3d_conf=None
    if use_3dpose:
        points3d = torch.tensor(keypoints3d, dtype=dtype)
        gt_joints3d = points3d[:, :, :3]
        joints3d_conf = points3d[:, :, 3].reshape(1, -1)
        gt_joints3d = gt_joints3d.to(device=device, dtype=dtype)
        joints3d_conf = joints3d_conf.to(device=device, dtype=dtype)

    # mesh gt 的形式转换
    use_mesh = kwargs.get('use_mesh', True)
    gt_mesh=None
    if use_mesh:
        gt_mesh = torch.tensor(mesh)
        gt_mesh = gt_mesh.to(device=device, dtype=dtype)
    ...
    # 添加mesh loss的key
    opt_weights_dict['mesh_loss_weight'] = mesh_loss_weights
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc='Orientation')):
            ...
            for opt_idx, curr_weights in enumerate(tqdm(opt_weights, desc='Stage')):
                ...
                closure = monitor.create_fitting_closure(
                    ...
                    gt_joints3d=gt_joints3d,joints3d_conf=joints3d_conf,
                    gt_mesh=gt_mesh,
                    joint_weights=joint_weights,
    ...
    if save_meshes or visualize:
        ...
        camera_rotation = camera.rotation.detach().cpu().numpy().squeeze()
        camera_transl = camera.translation.detach().cpu().numpy().squeeze()
        # 0.08249850559136471,0.9485327161660285
        # camera_transl += np.array([0,0,-3.2833183613412023 - 0])
        vertices = np.dot(vertices, camera_rotation)
        global_body_translation = np.array([ 0.33846804,  0.6681502 , -0.23373578], dtype=np.float32)
        body_scale = np.array([1.4774466], dtype=np.float32)
        vertices = vertices * body_scale + global_body_translation
        ...
        ########################### 顶点映射 ###############################
        import cv2
        oriImg_path = osp.join(kwargs.get("data_folder"),kwargs.get("img_folder"),osp.basename(mesh_fn).replace('obj','jpeg'))
        image = cv2.imread(oriImg_path)
        ...
        for i in range(projected_vshaped.shape[1]):
            ...
            cv2.circle(out_image, (x,y),1,(255,255,255),-1)
        ...
        for i in range(projected_joints.shape[1]):
            ...
            cv2.circle(out_image, (x,y),2,(0,0,255),-1)
        cv2.imwrite(mesh_fn.replace('obj','jpeg'), out_image)  
        ########################### 顶点映射 ###############################
        pdb.set_trace()
        # ########################### face映射 #############################
        # import cv2
